...
pub struct GameTextures<'a> {
    ...
    pub road_corner: Option<Texture<'a>>,
    pub grass: Option<Texture<'a>>,
}
...
impl<'a> GameTextures<'a> {
    pub fn load(creator: &'a TextureCreator<WindowContext>) -> Self {
        ...
        let road_left = load_texture_from_png(creator, Path::new("assets/road_left.png"))
            .map_err(|e| eprintln!("Warning: could not load assets/road_left.png: {}", e))
            .ok();
        let road_mid = load_texture_from_png(creator, Path::new("assets/road_mid.png"))
            .map_err(|e| eprintln!("Warning: could not load assets/road_mid.png: {}", e))
            .ok();
        let road_right = load_texture_from_png(creator, Path::new("assets/road_right.png"))
            .map_err(|e| eprintln!("Warning: could not load assets/road_right.png: {}", e))
            .ok();
        let road_corner = load_texture_from_png(creator, Path::new("assets/road_corner.png"))
            .map_err(|e| eprintln!("Warning: could not load assets/road_corner.png: {}", e))
            .ok();
        let grass = load_texture_from_png(creator, Path::new("assets/grass.png"))
            .map_err(|e| eprintln!("Warning: could not load assets/grass.png: {}", e))
            .ok();
...
pub fn render_scene(
    ...
    draw_grass_background(canvas, textures);
...
fn render_hud(
    ...
    let esc_w = esc_text.len() as i32 * 7;
    let esc_x = WINDOW_WIDTH as i32 - esc_w - 10;
    canvas.set_blend_mode(BlendMode::Blend);
    canvas.set_draw_color(Color::RGBA(15, 15, 15, 230));
    canvas
        .fill_rect(Rect::new(
            esc_x - 6,
            pad_y,
    ...
    for (i, (key, label, active, color)) in indicators.iter().enumerate() {
        ...
        let bg = if *active {
            Color::RGBA(color.r / 4, color.g / 4, color.b / 4, 240)
        } else {
            Color::RGBA(15, 15, 15, 230)
        };
        ...
        canvas
            .fill_rect(Rect::new(x + 2, y + 2, 14, (pill_h - 4) as u32))
            .ok();
...
fn draw_trajectory(canvas: &mut WindowCanvas, v: &Vehicle) {
    ...
    for (i, (x, y)) in points.iter().enumerate() {
        ...
        let py = *y as i32;
        canvas
            .fill_rect(sdl2::rect::Rect::new(px - 2, py - 2, 4, 4))
            .ok();
    }
}
...
fn draw_grass_background(canvas: &mut WindowCanvas, tex: &GameTextures) {
    let t = match &tex.grass {
        Some(t) => t,
        None => return,
    };
    let tw = tile_w();
    ...
    while y < WINDOW_HEIGHT as i32 {
        ...
        while x < WINDOW_WIDTH as i32 {
            let dst = Rect::new(x, y, tw as u32, tw as u32);
...
fn draw_roads(canvas: &mut WindowCanvas, tex: &GameTextures) {
    ...
    draw_road_corners(canvas, tex);
}

/// Draw curved corner tiles at the 4 grass-corner elbows adjacent to the intersection.
/// Each tile occupies a `tw`×`tw` cell in the grass area, touching the intersection
/// corner at its inner vertex. Asset's curve points to bottom-right → rotate per quadrant.
fn draw_road_corners(canvas: &mut WindowCanvas, tex: &GameTextures) {
    let t = match &tex.road_corner {
        Some(t) => t,
        None => return,
    };
    let tw = tile_w();
    ...
    let corners: [(i32, i32, f64); 4] = [
        (il, it, 180.0),         // top-left inside corner
        (ir - tw, it, 270.0),    // top-right
        (ir - tw, ib - tw, 0.0), // bottom-right
        (il, ib - tw, 90.0),     // bottom-left
    ];

    for (x, y, angle) in corners {
        let dst = Rect::new(x, y, tw as u32, tw as u32);
        canvas
            .copy_ex(t, None, Some(dst), angle, None, false, false)
            .ok();
    }
}
...
fn draw_dashed_line_v(canvas: &mut WindowCanvas, x: i32, y_start: i32, y_end: i32) {
    ...
    while y < y_end {
        ...
        canvas
            .fill_rect(Rect::new(x - 1, y, 2, (end - y) as u32))
            .ok();
        y += dash + gap;
    }
}
...
fn draw_dashed_line_h(canvas: &mut WindowCanvas, y: i32, x_start: i32, x_end: i32) {
    ...
    while x < x_end {
        ...
        canvas
            .fill_rect(Rect::new(x, y - 1, (end - x) as u32, 2))
            .ok();
        x += dash + gap;
    }
}
...
fn draw_solid_line_v(canvas: &mut WindowCanvas, x: i32, y_start: i32, y_end: i32) {
    canvas
        .fill_rect(Rect::new(x - 1, y_start, 3, (y_end - y_start) as u32))
        .ok();
}

fn draw_solid_line_h(canvas: &mut WindowCanvas, y: i32, x_start: i32, x_end: i32) {
    canvas
        .fill_rect(Rect::new(x_start, y - 1, (x_end - x_start) as u32, 3))
        .ok();
}
...
fn fill_triangle(canvas: &mut WindowCanvas, x0: i32, y0: i32, x1: i32, y1: i32, x2: i32, y2: i32) {
    ...
    for y in pts[0].1..=pts[2].1 {
        ...
        let x_short = if yf <= by {
            ...
            if seg_h < 0.5 {
                ...
                bx + (yf - by) / seg_h * (cx - bx)
            }
        };
        let (xs, xe) = if x_long <= x_short {
            (x_long, x_short)
        } else {
            (x_short, x_long)
        };
        canvas
            .draw_line(Point::new(xs as i32, y), Point::new(xe as i32, y))
            .ok();
    }
}
...
pub fn render_statistics(canvas: &mut WindowCanvas, stats: &Statistics) {
    ...
    canvas
        .draw_rect(Rect::new(box_x + 1, box_y + 1, box_w - 2, box_h - 2))
        .ok();

    let stat_items: Vec<(&str, f64, Color)> = vec![
        (
            "Vehicles Passed",
            stats.total_vehicles as f64,
        ...
        (
            "Avg Wait to Enter",
            stats.avg_wait_to_enter,
            Color::RGB(100, 220, 200),
        ),
        (
            "Avg Transit Time",
            stats.avg_transit_time,
            Color::RGB(220, 160, 100),
        ),
        (
            "Close Calls",
            stats.close_calls as f64,
            Color::RGB(255, 80, 80),
        ),
    ];
    ...
    for (i, (_, _, color)) in stat_items.iter().enumerate() {
        ...
        canvas
            .fill_rect(Rect::new(
                box_x + padding,
                y,
                box_w - padding as u32 * 2,
                item_h as u32 - 8,
            ))
            .ok();
        canvas.set_draw_color(*color);
        canvas
            .fill_rect(Rect::new(box_x + padding, y, 6, item_h as u32 - 8))
            .ok();
    }
    ...
    render_text_block(
        ...
        WINDOW_WIDTH as i32 / 2 - 100,
        box_y + box_h as i32 + 30,
        1,
    );
}
